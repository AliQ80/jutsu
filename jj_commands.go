			...
			Commands: []Command{
				...
				},
				{
					...
					Flags: []Flag{
						...
						{Name: "ignore-all-space", Description: "Ignore whitespace when comparing lines", Value: "--ignore-all-space"},
						{Name: "ignore-space-change", Description: "Ignore changes in amount of whitespace when comparing lines", Value: "--ignore-space-change"},
						{Name: "tool", Description: "Generate diff by external command\n\nA builtin format can also be specified as `:<name>`. For example, `--tool=:git` is\nequivalent to `--git`.", Value: "--tool", RequiresInput: true, InputType: "TOOL"},
				...
				},
				{
					...
					Flags: []Flag{
						{Name: "template", Description: "Render each revision using the given template\n\nYou can specify arbitrary template expressions using the [built-in keywords]. See [`jj\nhelp -k templates`] for more information.", Value: "-T", RequiresInput: true, InputType: "TEMPLATE"},
						...
						{Name: "git", Description: "Show a Git-format diff", Value: "--git", ConflictingFlags: []string{"--color-words"}},
						{Name: "color-words", Description: "Show a word-level diff with changes indicated only by color", Value: "--color-words", ConflictingFlags: []string{"--git"}},
						{Name: "ignore-all-space", Description: "Ignore whitespace when comparing lines", Value: "-w"},
						{Name: "ignore-space-change", Description: "Ignore changes in amount of whitespace when comparing lines", Value: "-b"},
						{Name: "tool", Description: "Generate diff by external command\n\nA builtin format can also be specified as `:<name>`. For example, `--tool=:git` is\nequivalent to `--git`.", Value: "--tool", RequiresInput: true, InputType: "TOOL"},
						{Name: "context", Description: "Number of lines of context to show", Value: "--context", RequiresInput: true, InputType: "CONTEXT"},
					},
				},
				{
					Name:        "diff",
					Description: "Compare file contents between two revisions\n\nWith the `-r` option, shows the changes compared to the parent revision. If there are several parent\nrevisions (i.e., the given revision is a merge), then they will be merged and the changes from the\nresult to the given revision will be shown.\n\nWith the `--from` and/or `--to` options, shows the difference from/to the given revisions. If either\nis left out, it defaults to the working-copy commit. For example, `jj diff --from main` shows the\nchanges from \"main\" (perhaps a bookmark name) to the working-copy commit.\n\nIf no option is specified, it defaults to `-r @`.",
					Args: []Arg{
				...
				{
					Name:        "interdiff",
					Description: "Show differences between the diffs of two revisions\n\nThis is like running `jj diff -r` on each change, then comparing those results. It answers: \"How do\nthe modifications introduced by revision A differ from the modifications introduced by revision B?\"\n\nFor example, if two changes both add a feature but implement it differently, `jj interdiff --from @-\n--to other` shows what one implementation adds or removes that the other doesn't.\n\nA common use of this command is to compare how a change has changed since the last push to a remote:\n\n```sh $ jj interdiff --from push-xyz@origin --to push-xyz ```\n\nThis command is different from `jj diff --from A --to B`, which compares file contents directly.\n`interdiff` compares what the changes do in terms of their patches, rather than their file contents.\nThis makes a difference when the two revisions have different parents: `jj diff --from A --to B`\nwill include the changes between their parents while `jj interdiff --from A --to B` will not.\n\nTechnically, this works by rebasing `--from` onto `--to`'s parents and comparing the result to\n`--to`.\n\nTo see the changes throughout the whole evolution of a change instead of between just two revisions,\nuse `jj evolog -p` instead.",
					Args: []Arg{
						{Name: "FILESETS", Description: "Restrict the diff to these paths", Variadic: true},
					},
					Flags: []Flag{
					...
					SubCmds: []SubCommand{
						{
							...
							Flags: []Flag{
								{Name: "revision", Description: "The revision to list files in\n\n[default: @]", Value: "-r", RequiresInput: true, InputType: "REVSET"},
								{Name: "template", Description: "Render each file entry using the given template\n\nAll 0-argument methods of the [`TreeEntry` type] are available as keywords in the template\nexpression. See [`jj help -k templates`] for more information.", Value: "-T", RequiresInput: true, InputType: "TEMPLATE"},
							},
						},
						{
							Name:        "show",
							...
							Flags: []Flag{
								...
								{Name: "template", Description: "Render each file metadata using the given template\n\nAll 0-argument methods of the [`TreeEntry` type] are available as keywords in the template\nexpression. See [`jj help -k templates`] for more information.\n\nIf not specified, this defaults to the `templates.file_show` setting.", Value: "-T", RequiresInput: true, InputType: "TEMPLATE"},
							},
						},
						{
							Name:        "annotate",
							Description: "Show the source change for each line of the target file.\n\nAnnotates a revision line by line. Each line includes the source change that introduced the\nassociated line. A path to the desired file must be provided.",
							Args: []Arg{
							...
							Flags: []Flag{
								...
								{Name: "template", Description: "Render each line using the given template\n\nAll 0-argument methods of the [`AnnotationLine` type] are available as keywords in the\ntemplate expression. See [`jj help -k templates`] for more information.\n\nIf not specified, this defaults to the `templates.file_annotate` setting.", Value: "-T", RequiresInput: true, InputType: "TEMPLATE"},
							},
						},
						{
							Name:        "search",
							Description: "Search for content in files\n\nLists files containing the specified pattern.\n\nThis is an early version of the command. It only supports glob matching for now, it doesn't search\nfiles concurrently, and it doesn't indicate where in the file the match was found.",
							Args: []Arg{
								{Name: "FILESETS", Description: "Only search files matching these prefixes (instead of all files)", Variadic: true},
							},
							Flags: []Flag{
								{Name: "pattern", Description: "The pattern to search for in a single line\n\nIt is a [string pattern syntax] like `kind:pattern`.  The kind defaults to regex when\nomitted.\n\nIf it is a glob pattern, the whole line must match the pattern, so you may want to pass\nsomething like `--pattern 'glob:*foo*'`.", Value: "--pattern", RequiresInput: true, Mandatory: true, Selected: true, InputType: "PATTERN"},
			...
			Commands: []Command{
				{
					Name:        "commit",
					Alias:       "ci",
					Description: "Update the description and create a new change on top [default alias: ci]\n\nWhen called without path arguments or `--interactive`, `jj commit` is equivalent to `jj describe`\nfollowed by `jj new`.\n\nWhen using `--interactive` or path arguments, the selected changes stay in the current commit while\nthe remaining changes are moved to a new working-copy commit on top. This is very similar to `jj\nsplit`. Differences include:\n\n* `jj commit` is not interactive by default (it selects all changes).\n\n* `jj commit` doesn't have a `-r` option. It always acts on the working-copy commit (@).\n\n* `jj split` (without `-o`/`-A`/`-B`) will move bookmarks forward from the old change to the child\nchange. `jj commit` doesn't move bookmarks forward.\n\n* `jj split` allows you to move the selected changes to a different destination with `-o`/`-A`/`-B`.",
					Args: []Arg{
				...
				{
					Name:        "new",
					Description: "Create a new, empty change and (by default) edit it in the working copy\n\nBy default, `jj` will edit the new change, making the [working copy] represent the new commit. This\ncan be avoided with `--no-edit`.\n\nNote that you can create a merge commit by specifying multiple revisions as argument. For example,\n`jj new @ main` will create a new commit with the working copy and the `main` bookmark as parents.",
					Args: []Arg{
				...
				},
				{
					...
					Flags: []Flag{
						...
						{Name: "edit", Description: "Instead of creating a new working-copy commit on top of the target commit (like `jj new`),\nedit the target commit directly (like `jj edit`)\n\nTakes precedence over config in `ui.movement.edit`; i.e. will negate `ui.movement.edit =\nfalse`", Value: "--edit", ConflictingFlags: []string{"--no-edit"}},
						{Name: "no-edit", Description: "The inverse of `--edit`\n\nTakes precedence over config in `ui.movement.edit`; i.e. will negate `ui.movement.edit =\ntrue`", Value: "--no-edit", ConflictingFlags: []string{"--edit"}},
				...
				{
					Name:        "resolve",
					Description: "Resolve conflicted files with an external merge tool\n\nOnly conflicts that can be resolved with a 3-way merge are supported. See docs for merge tool\nconfiguration instructions. External merge tools will be invoked for each conflicted file one-by-one\nuntil all conflicts are resolved. To stop resolving conflicts, exit the merge tool without making\nany changes.\n\nNote that conflicts can also be resolved without using this command. You may edit the conflict\nmarkers in the conflicted file directly with a text editor.",
					Args: []Arg{
						{Name: "FILESETS", Description: "Only resolve conflicts in these paths. You can use the `--list` argument to find paths to\nuse here", Variadic: true},
					},
					Flags: []Flag{
						{Name: "list", Description: "Instead of resolving conflicts, list all the conflicts", Value: "--list"},
						{Name: "revision", Description: "[default: @]", Value: "-r", RequiresInput: true, InputType: "REVSET"},
					},
				},
				{
					Name:        "restore",
					Description: "Restore paths from another revision\n\nThat means that the paths get the same content in the destination (`--into`) as they had in the\nsource (`--from`). This is typically used for undoing changes to some paths in the working copy (`jj\nrestore <paths>`).\n\nIf only one of `--from` or `--into` is specified, the other one defaults to the working copy.\n\nWhen neither `--from` nor `--into` is specified, the command restores into the working copy from its\nparent(s). `jj restore` without arguments is similar to `jj abandon`, except that it leaves an empty\nrevision with its description and other metadata preserved.\n\nSee `jj diffedit` if you'd like to restore portions of files rather than entire files.",
					Args: []Arg{
					...
					Flags: []Flag{
						...
						{Name: "into", Description: "Revision to restore into (destination)\n\n[aliases: --to]", Value: "--into", RequiresInput: true, InputType: "REVSET"},
				...
				},
				{
					...
					Flags: []Flag{
						{Name: "source", Description: "Fix files in the specified revision(s) and their descendants. If no revisions are\nspecified, this defaults to the `revsets.fix` setting, or `reachable(@, mutable())` if it\nis not set", Value: "--source", RequiresInput: true, InputType: "REVSETS"},
						{Name: "include-unchanged-files", Description: "Fix unchanged files in addition to changed ones. If no paths are specified, all files in\nthe repo will be fixed", Value: "--include-unchanged-files"},
						{Name: "all-lines", Description: "Format all lines instead of only modified lines.\n\nIf the formatter doesn't support formatting only modified lines, then this option has no\neffect since the formatter always formats all lines.", Value: "--all-lines"},
					},
				},
				{
					Name:        "file",
					...
					SubCmds: []SubCommand{
						{
							Name:        "track",
							Description: "Start tracking specified paths in the working copy\n\nWithout arguments, all paths that are not ignored will be tracked.\n\nBy default, new files in the working copy are automatically tracked, so this command has no effect.\nYou can configure which paths to automatically track by setting `snapshot.auto-track` (e.g. to\n`\"none()\"` or `\"glob:**/*.rs\"`). Files that don't match the pattern can be manually tracked using\nthis command. The default pattern is `all()`.",
							Args: []Arg{
								{Name: "FILESETS", Description: "Paths to track", Variadic: true, Required: true},
							},
							Flags: []Flag{
								{Name: "include-ignored", Description: "Track paths even if they're ignored or too large\n\nBy default, `jj file track` will not track files that are ignored by .gitignore or exceed\nthe maximum file size. This flag overrides those restrictions, explicitly tracking the\nspecified paths.", Value: "--include-ignored"},
							},
						},
						{
							Name:        "untrack",
			...
			Commands: []Command{
					...
					SubCmds: []SubCommand{
						{
							Name:        "list",
							Alias:       "l",
							Description: "List bookmarks and their targets\n\nBy default, a tracked remote bookmark will be included only if its target is different from the\nlocal target. An untracked remote bookmark won't be listed. For a conflicted bookmark (both local\nand remote), old target revisions are preceded by a \"-\" and new target revisions are preceded by a\n\"+\".\n\nSee [`jj help -k bookmarks`] for more information.",
							Args: []Arg{
								{Name: "NAMES", Description: "Show bookmarks whose local name matches\n\nBy default, the specified pattern matches bookmark names with glob syntax. You can also\nuse other [string pattern syntax].", Variadic: true},
							},
							Flags: []Flag{
								{Name: "all-remotes", Description: "Show all tracked and untracked remote bookmarks including the ones whose targets are\nsynchronized with the local bookmarks", Value: "-a"},
								{Name: "remote", Description: "Show all tracked and untracked remote bookmarks belonging to this remote\n\nCan be combined with `--tracked` or `--conflicted` to filter the bookmarks shown (can be\nrepeated.)\n\nBy default, the specified pattern matches remote names with glob syntax. You can also use\nother [string pattern syntax].", Value: "--remote", RequiresInput: true, InputType: "REMOTE"},
								...
								{Name: "revision", Description: "Show bookmarks whose local targets are in the given revisions\n\nNote that `-r deleted_bookmark` will not work since `deleted_bookmark` wouldn't have a\nlocal target.", Value: "-r", RequiresInput: true, InputType: "REVSETS"},
							},
						},
						{
							Name:        "advance",
							Alias:       "a",
							Description: "Advance the closest bookmarks to a target revision\n\nThe target `--to` defaults to `revsets.bookmark-advance-to` (which defaults to `@`).\n\nThe bookmarks to advance are determined by `revsets.bookmark-advance-from` (which defaults to\n`heads(::to & bookmarks())`).\n\nNote that the from revset has access to `to`.\n\nPositional bookmark name arguments can target specific bookmarks to advance to the target, in this\ncase the default from revset is ignored.\n\nExample:\n\n`jj bookmark advance --to x` - Does the equivalent of `jj bookmark move --from 'heads(::x &\nbookmarks())' --to x`.",
							Args: []Arg{
						...
						},
						{
							...
							Flags: []Flag{
								...
								{Name: "to", Description: "Move bookmarks to this revision\n\n[default: @]", Value: "--to", RequiresInput: true, InputType: "REVSET"},
						...
						},
						{
							Name:        "track",
							Alias:       "t",
							Description: "Start tracking given remote bookmarks\n\nA tracked remote bookmark will be imported as a local bookmark of the same name. Changes to it will\npropagate to the existing local bookmark on future pulls.",
							Args: []Arg{
								{Name: "BOOKMARK", Description: "Bookmark names to track\n\nBy default, the specified pattern matches bookmark names with glob syntax. You can also\nuse other [string pattern syntax].", Variadic: true, Required: true},
							},
							Flags: []Flag{
								{Name: "remote", Description: "Remote names to track\n\nBy default, the specified pattern matches remote names with glob syntax. You can also use\nother [string pattern syntax].\n\nIf no remote names are given, all remote bookmarks matching the bookmark names will be\ntracked.", Value: "--remote", RequiresInput: true, InputType: "REMOTE"},
							},
						},
						{
							Name:        "untrack",
							Description: "Stop tracking given remote bookmarks\n\nAn untracked remote bookmark is just a pointer to the last-fetched remote bookmark. It won't be\nimported as a local bookmark on future pulls.\n\nIf you want to forget a local bookmark while also untracking the corresponding remote bookmarks, use\n`jj bookmark forget` instead.",
							Args: []Arg{
								{Name: "BOOKMARK", Description: "Bookmark names to untrack\n\nBy default, the specified pattern matches bookmark names with glob syntax. You can also\nuse other [string pattern syntax].", Variadic: true, Required: true},
							},
							Flags: []Flag{
								{Name: "remote", Description: "Remote names to untrack\n\nBy default, the specified pattern matches remote names with glob syntax. You can also use\nother [string pattern syntax].\n\nIf no remote names are given, all remote bookmarks matching the bookmark names will be\nuntracked.", Value: "--remote", RequiresInput: true, InputType: "REMOTE"},
							},
						},
						{
							Name:        "forget",
							Alias:       "f",
							Description: "Forget a bookmark without marking it as a deletion to be pushed\n\nIf a local bookmark is forgotten, any corresponding remote bookmarks will become untracked to ensure\nthat the forgotten bookmark will not impact remotes on future pushes.",
							Args: []Arg{
								{Name: "NAMES", Description: "The bookmarks to forget\n\nBy default, the specified pattern matches bookmark names with glob syntax. You can also\nuse other [string pattern syntax].", Variadic: true, Required: true},
							},
							Flags: []Flag{
								{Name: "include-remotes", Description: "When forgetting a local bookmark, also forget any corresponding remote bookmarks\n\nA forgotten remote bookmark will not impact remotes on future pushes. It will be recreated\non future fetches if it still exists on the remote. If there is a corresponding\nGit-tracking remote bookmark, it will also be forgotten.", Value: "--include-remotes"},
							},
						},
						{
							Name:        "delete",
				...
				},
				{
					...
					SubCmds: []SubCommand{
						{
							Name:        "fetch",
							Description: "Fetch from a Git remote\n\nIf no remotes are specified, fetches the remotes specified by the `git.fetch` setting. If that is\nnot configured and there are multiple remotes, the remote named \"origin\" will be used.\n\nIf no branches nor tags are specified, fetches bookmarks and tags specified by the\n`remotes.<name>.fetch-bookmarks`/`fetch-tags` settings. If `remotes.<name>.fetch-bookmarks` is not\nconfigured, the default fetch refspecs for the selected remotes are read from the Git configuration.\n\nCommits that are no longer reachable from any branch on the remote will be considered abandoned by\nthe remote, and will be abandoned in the local repo to match the remote. Set\n`git.abandon-unreachable-commits` to `false` to disable this behavior.\n\nIf a working-copy commit gets abandoned, it will be given a new, empty commit. This is true in\ngeneral; it is not specific to this command.",
							Flags: []Flag{
								{Name: "remote", Description: "The remote to fetch from (only named remotes are supported, can be repeated)\n\nBy default, the specified pattern matches remote names with glob syntax, e.g. `--remote\n'*'`. You can also use other [string pattern syntax].", Value: "--remote", RequiresInput: true, ConflictingFlags: []string{"--all-remotes"}, InputType: "REMOTE"},
								{Name: "all-remotes", Description: "Fetch from all remotes", Value: "--all-remotes", ConflictingFlags: []string{"--remote"}},
								{Name: "branch", Description: "Name of the branch to fetch (can be repeated)\n\nBy default, the specified pattern matches branch names with glob syntax, but only `*` is\nexpanded. Other wildcard characters such as `?` are *not* supported. Patterns can be\nrepeated or combined with [logical operators] to specify multiple branches, but only union\nand negative intersection are supported.\n\nExamples: `push-*`, `(push-* | foo/*) ~ foo/unwanted`", Value: "--branch", RequiresInput: true, InputType: "BRANCH"},
						...
						},
						{
							...
							Flags: []Flag{
								...
								{Name: "change", Description: "Push this commit by creating a bookmark (can be repeated)\n\nThe created bookmark will be tracked automatically. Use the `templates.git_push_bookmark`\nsetting to customize the generated bookmark name. The default is `\"push-\" ++\nchange_id.short()`.", Value: "-c", RequiresInput: true, InputType: "REVSETS"},
								{Name: "named", Description: "Specify a new bookmark name and a revision to push under that name, e.g. '--named\nmyfeature=@'\n\nAutomatically tracks the bookmark if it is new.", Value: "--named", RequiresInput: true},
								{Name: "dry-run", Description: "Only display what will change on the remote", Value: "--dry-run"},
							},
						},
						{
							Name:        "import",
							Description: "Update repo with changes made in the underlying Git repo\n\nCommits that are no longer reachable from any branch in the Git repo will be considered abandoned in\nthe Git repo, and will be abandoned in the jj repo to match the Git repo. Set\n`git.abandon-unreachable-commits` to `false` to disable this behavior.\n\nIf a working-copy commit gets abandoned, it will be given a new, empty commit. This is true in\ngeneral; it is not specific to this command.\n\nThere is no need to run this command if you're in colocated workspace because the import happens\nautomatically there.",
							Flags:       []Flag{},
						},
						{
							Name:        "export",
							Description: "Update the underlying Git repo with changes made in the repo\n\nThere is no need to run this command if you're in colocated workspace because the export happens\nautomatically there.",
							Flags:       []Flag{},
						},
						{
							Name:        "remote add",
					...
					SubCmds: []SubCommand{
						{
							Name:        "list",
							Alias:       "l",
							Description: "List tags and their targets\n\nBy default, a tracked remote tag will be included only if its target is different from the local\ntag. An untracked remote tag won't be listed. For a conflicted tag (both local and remote), old\ntarget revisions are preceded by a \"-\" and new target revisions are preceded by a \"+\".\n\nThe `-r` flag combined with revset expressions can be used for filtering. For example:\n\n* `jj tag list -r 'REV::'` shows tags whose targets are descendants of REV (similar to `git tag\n--contains REV`).\n\n* `jj tag list -r '::REV'` shows tags whose targets are ancestors of REV (similar to `git tag\n--merged REV`).",
							Args: []Arg{
								{Name: "NAMES", Description: "Show tags whose local name matches\n\nBy default, the specified pattern matches tag names with glob syntax. You can also use\nother [string pattern syntax].", Variadic: true},
							},
							Flags: []Flag{
								{Name: "all-remotes", Description: "Show all tracked and untracked remote tags including the ones whose targets are\nsynchronized with the local tags", Value: "-a"},
								{Name: "remote", Description: "Filter by remote", Value: "--remote", RequiresInput: true},
								{Name: "tracked", Description: "Show tracked tags only", Value: "--tracked"},
							},
						},
						{
							Name:        "set",
							...
							Flags: []Flag{
								...
								{Name: "allow-backward", Description: "Allow moving tag backward", Value: "--allow-backward"},
							},
						},
						{
							Name:        "delete",
							...
							Args: []Arg{
								{Name: "NAMES", Description: "Tag names to delete\n\nBy default, the specified pattern matches tag names with glob syntax. You can also use\nother [string pattern syntax].", Variadic: true, Required: true},
							},
							Flags: []Flag{},
						},
					},
			...
			Commands: []Command{
				...
				},
				{
					...
					Flags: []Flag{
						...
						{Name: "destination", Description: "Destination revision (onto)\n\n[aliases: -d]", Value: "-d", RequiresInput: true, ConflictingFlags: []string{"--insert-after", "--insert-before"}},
						{Name: "insert-after", Description: "The revision(s) to insert after (can be repeated to create a merge commit)\n\n[aliases: --after]", Value: "--insert-after", RequiresInput: true, ConflictingFlags: []string{"-d"}, InputType: "REVSETS"},
						{Name: "insert-before", Description: "The revision(s) to insert before (can be repeated to create a merge commit)\n\n[aliases: --before]", Value: "--insert-before", RequiresInput: true, ConflictingFlags: []string{"-d"}, InputType: "REVSETS"},
						{Name: "skip-emptied", Description: "If true, when rebasing would produce an empty commit, the commit is abandoned. It will not\nbe abandoned if it was already empty before the rebase. Will never skip merge commits with\nmultiple non-empty parents", Value: "--skip-emptied"},
				...
				{
					Name:        "absorb",
					Description: "Move changes from a revision into the stack of mutable revisions\n\nThis command splits changes in the source revision and moves each change to the closest mutable\nancestor where the corresponding lines were modified last. If the destination revision cannot be\ndetermined unambiguously, the change will be left in the source revision.\n\nThe source revision will be abandoned if all changes are absorbed into the destination revisions,\nand if the source revision has no description.\n\nThe modification made by `jj absorb` can be reviewed by `jj op show -p`.",
					Args: []Arg{
						{Name: "FILESETS", Description: "Move only changes to these paths (instead of all paths)", Variadic: true},
					},
					Flags: []Flag{
						{Name: "from", Description: "Source revision to absorb from\n\n[default: @]", Value: "-f", RequiresInput: true, InputType: "REVSET"},
						{Name: "into", Description: "Destination revisions to absorb into\n\nOnly ancestors of the source revision will be considered.\n\n[default: mutable()]\n[aliases: --to]", Value: "-t", RequiresInput: true, InputType: "REVSETS"},
					},
				},
				{
					Name:        "abandon",
					Description: "Abandon a revision\n\nAbandon a revision, rebasing descendants onto its parent(s). The behavior is similar to `jj restore\n--changes-in`; the difference is that `jj abandon` gives you a new change, while `jj restore`\nupdates the existing change.\n\nIf a working-copy commit gets abandoned, it will be given a new, empty commit. This is true in\ngeneral; it is not specific to this command.",
					Args: []Arg{
				...
				{
					Name:        "duplicate",
					Description: "Create new changes with the same content as existing ones\n\nWhen none of the `--onto`, `--insert-after`, or `--insert-before` arguments are provided, commits\nwill be duplicated onto their existing parents or onto other newly duplicated commits.\n\nWhen any of the `--onto`, `--insert-after`, or `--insert-before` arguments are provided, the roots\nof the specified commits will be duplicated onto the destination indicated by the arguments. Other\nspecified commits will be duplicated onto these newly duplicated commits. If the `--insert-after` or\n`--insert-before` arguments are provided, the new children indicated by the arguments will be\nrebased onto the heads of the specified commits.\n\nBy default, the duplicated commits retain the descriptions of the originals. This can be customized\nwith the `templates.duplicate_description` setting.",
					Args: []Arg{
				...
				},
				{
					...
					Description: "Simplify parent edges for the specified revision(s).\n\nRemoves all parents of each of the specified revisions that are also indirect ancestors of the same\nrevisions through other parents. This has no effect on any revision's contents, including the\nworking copy.\n\nIn other words, for all (A, B, C) where A has (B, C) as parents and C is an ancestor of B, A will be\nrewritten to have only B as a parent instead of B+C.",
					Flags: []Flag{
						{Name: "revision", Description: "Simplify specified revision(s) (can be repeated)\n\nIf both `--source` and `--revisions` are not provided, this defaults to the\n`revsets.simplify-parents` setting, or `reachable(@, mutable())` if it is not set.", Value: "-r", RequiresInput: true, InputType: "REVSETS"},
			...
			Commands: []Command{
				{
					...
					SubCmds: []SubCommand{
						{
							Name:        "log",
							Description: "Show the operation log\n\nLike other commands, `jj op log` snapshots the current working-copy changes and reconciles divergent\noperations. Use `--at-op=@ --ignore-working-copy` to inspect the current state without mutation.",
							Flags: []Flag{
						...
						},
						{
							...
							Flags: []Flag{
								{Name: "operation", Description: "Show repository changes in this operation, compared to its parent\n\n[aliases: --op]", Value: "--operation", RequiresInput: true, InputType: "OPERATION"},
								...
								{Name: "types", Description: "For each path, show only its type before and after\n\nThe diff is shown as two letters. The first letter indicates the type before and the\nsecond letter indicates the type after. '-' indicates that the path was not present, 'F'\nrepresents a regular file, `L' represents a symlink, 'C' represents a conflict, and 'G'\nrepresents a Git submodule.", Value: "--types", ConflictingFlags: []string{"--summary", "--stat", "--name-only"}},
								{Name: "name-only", Description: "For each path, show only its path\n\nTypically useful for shell commands like: `jj diff -r @- --name-only | xargs perl -pi\n-e's/OLD/NEW/g`", Value: "--name-only", ConflictingFlags: []string{"--summary", "--stat", "--types"}},
								{Name: "git", Description: "Show a Git-format diff", Value: "--git", ConflictingFlags: []string{"--color-words"}},
								{Name: "color-words", Description: "Show a word-level diff with changes indicated only by color", Value: "--color-words", ConflictingFlags: []string{"--git"}},
								{Name: "tool", Description: "Generate diff by external command\n\nA builtin format can also be specified as `:<name>`. For example, `--tool=:git` is\nequivalent to `--git`.", Value: "--tool", RequiresInput: true, InputType: "TOOL"},
								{Name: "context", Description: "Number of lines of context to show", Value: "--context", RequiresInput: true, InputType: "CONTEXT"},
						...
						{
							Name:        "restore",
							Description: "Create a new operation that restores the repo to an earlier state\n\nThis restores the repo to the state at the specified operation, effectively undoing all later\noperations. It does so by creating a new operation.",
							Args: []Arg{
								{Name: "OPERATION", Description: "The operation to restore to\n\nUse `jj op log` to find an operation to restore to. Use e.g. `jj --at-op=<operation ID>\nlog` before restoring to an operation to see the state of the repo at that operation.", Required: true},
							},
							Flags: []Flag{
								{Name: "what", Description: "What portions of the local state to restore (can be repeated)\n\nThis option is EXPERIMENTAL.\n\nPossible values:\n- repo:            The jj repo state and local bookmarks\n- remote-tracking: The remote-tracking bookmarks. Do not restore these if you'd like to\npush after the undo\n\n[default: repo remote-tracking]", Value: "--what", RequiresInput: true, InputType: "WHAT"},
							},
						},
						{
							Name:        "revert",
						...
						{
							Name:        "abandon",
							Description: "Abandon operation history\n\nTo discard old operation history, use `jj op abandon ..<operation ID>`. It will abandon the\nspecified operation and all its ancestors. The descendants will be reparented onto the root\noperation.\n\nTo discard recent operations, use `jj op restore <operation ID>` followed by `jj op abandon\n<operation ID>..@-`.\n\nPrevious versions of a change (or predecessors) are also discarded if they become unreachable from\nthe operation history. The abandoned operations, commits, and other unreachable objects can later be\ngarbage collected by using `jj util gc` command.",
							Args: []Arg{
						...
						{
							Name:        "integrate",
							Description: "Make an operation part of the operation log\n\nBy default, operations are automatically integrated into the operation log, but\n`--no-integrate-operation` or internal errors may cause that to not happen. This command can then be\nused for making such operations part of the operation log.\n\nRunning this command on an operation that is already in the operation log (`jj op log`) has no\neffect.",
							Args: []Arg{
				...
				{
					Name:        "redo",
					Description: "Redo the most recently undone operation\n\nThis is the natural counterpart of `jj undo`. Repeated invocations of `jj undo` and `jj redo` act\nsimilarly to Undo/Redo commands in a text editor.\n\nUse `jj op log` to visualize the log of past operations, including a detailed description of any\npast undo/redo operations. See also `jj op restore` to explicitly restore an older operation by its\nid (available in the operation log).",
					Flags:       []Flag{},
				},
			},
		},
		{
			...
			Commands: []Command{
				...
				},
				{
					...
					Flags: []Flag{
						{Name: "revision", Description: "What revision(s) to sign\n\nIf no revisions are specified, this defaults to the `revsets.sign` setting.\n\nNote that revisions are always re-signed.\n\nWhile that leads to discomfort for users, which sign with hardware devices, as of now we\ncannot reliably check if a commit is already signed by the user without creating a\nsignature (see [#5786]).", Value: "-r", RequiresInput: true, InputType: "REVSETS"},
				...
				},
				{
					...
					SubCmds: []SubCommand{
						{
							Name:        "list",
							Description: "List the patterns that are currently present in the working copy\n\nBy default, a newly cloned or initialized repo will have have a pattern matching all files from the\nrepo root. That pattern is rendered as `.` (a single period).",
							Flags:       []Flag{},
						},
						{
							Name:        "set",
							Description: "Update the patterns that are present in the working copy\n\nFor example, if all you need is the `README.md` and the `lib/` directory, use `jj sparse set --clear\n--add README.md --add lib`. If you no longer need the `lib` directory, use `jj sparse set --remove\nlib`.",
							Flags: []Flag{
								{Name: "patterns", Description: "Patterns to set (replaces current)", Value: "--patterns", RequiresInput: true},
				...
				},
				{
					...
					SubCmds: []SubCommand{
						...
						},
						{
							Name:        "path",
							Alias:       "p",
							Description: "Print the paths to the config files\n\nA config file at that path may or may not exist.\n\nIf `--repo` or `--workspace` is specified and the config file does not exist, jj will generate a new\nconfig directory for this repo/workspace and print the path to the config file in that directory.\n\nSee `jj config edit` if you'd like to immediately edit a file.",
							Flags: []Flag{
								{Name: "user", Description: "Target the user-level config", Value: "--user", ConflictingFlags: []string{"--repo", "--workspace"}},
								{Name: "repo", Description: "Target the repo-level config", Value: "--repo", ConflictingFlags: []string{"--user", "--workspace"}},
								{Name: "workspace", Description: "Target the workspace-level config", Value: "--workspace", ConflictingFlags: []string{"--user", "--repo"}},
						...
						},
						{
							...
							Args: []Arg{
								{Name: "NAME", Description: "", Required: true},
								{Name: "VALUE", Description: "New value to set\n\nThe value should be specified as a TOML expression. If string value isn't enclosed by any\nTOML constructs (such as apostrophes or array notation), quotes can be omitted. Note that\nthe value may also need shell quoting. TOML multi-line strings can be useful if the value\ncontains apostrophes. For example, to set `foo.bar` to the string \"{don't}\" use `jj config\nset --user foo.bar \"'''{don't}'''\"`. This is valid in both Bash and Fish.\n\nAlternative, e.g. to avoid dealing with shell quoting, use `jj config edit` to edit the\nTOML file directly.", Required: true},
							},
							Flags: []Flag{
								{Name: "user", Description: "Target the user-level config", Value: "--user", ConflictingFlags: []string{"--repo", "--workspace"}},
								{Name: "repo", Description: "Target the repo-level config", Value: "--repo", ConflictingFlags: []string{"--user", "--workspace"}},
				...
				{
					Name:        "workspace",
					Description: "Commands for working with workspaces\n\nWorkspaces let you add additional working copies attached to the same repo. A common use case is so\nyou can run a slow build or test in one workspace while you're continuing to write code in another\nworkspace.\n\nEach workspace has its own working-copy commit. When you have more than one workspace attached to a\nrepo, they are indicated by `<workspace name>@` in `jj log`.\n\nEach workspace also has own sparse patterns.",
					SubCmds: []SubCommand{
						{
							Name:        "list",
						...
						{
							Name:        "forget",
							Description: "Stop tracking a workspace's working-copy commit in the repo\n\nThe workspace will not be touched on disk. It can be deleted from disk before or after running this\ncommand.",
							Args: []Arg{
				...
				},
				{
					...
					SubCmds: []SubCommand{
						{
							Name:        "gc",
							Description: "Run backend-dependent garbage collection.\n\nTo garbage-collect old operations and the commits/objects referenced by them, run `jj op abandon\n..<some old operation>` before `jj util gc`.",
							Flags: []Flag{
								{Name: "expire", Description: "Time threshold\n\nBy default, only obsolete objects and operations older than 2 weeks are pruned.\n\nOnly the string \"now\" can be passed to this parameter. Support for arbitrary absolute and\nrelative timestamps will come in a subsequent release.", Value: "--expire", RequiresInput: true, InputType: "EXPIRE"},
							},
						},
						{
					...
					SubCmds: []SubCommand{
						{
							...
							Args: []Arg{
								...
								{Name: "DESTINATION", Description: "Specifies the target directory for the Jujutsu repository clone. If not provided, defaults\nto a directory named after the last component of the source URL. The full directory path\nwill be created if it doesn't exist"},
							},
							Flags: []Flag{
								{Name: "depth", Description: "Create a shallow clone of the given depth", Value: "--depth", RequiresInput: true, InputType: "DEPTH"},
								{Name: "colocate", Description: "Colocate the Jujutsu repo with the git repo\n\nSpecifies that the `jj` repo should also be a valid `git` repo, allowing the use of both\n`jj` and `git` commands in the same directory.\n\nThe repository will contain a `.git` dir in the top-level. Regular Git tools will be able\nto operate on the repo.\n\n**This is the default**, and this option has no effect, unless the [git.colocate config]\nis set to `false`.", Value: "--colocate"},
							},
						},
						{
							Name:        "init",
							...
							Args: []Arg{
								{Name: "DESTINATION", Description: "The destination directory where the `jj` repo will be created. If the directory does not\nexist, it will be created. If no directory is given, the current directory is used.\n\nBy default the `git` repo is under `$destination/.jj`\n\n[default: .]"},
							},
							Flags: []Flag{
								{Name: "colocate", Description: "Colocate the Jujutsu repo with the git repo\n\nSpecifies that the `jj` repo should also be a valid `git` repo, allowing the use of both\n`jj` and `git` commands in the same directory.\n\nThe repository will contain a `.git` dir in the top-level. Regular Git tools will be able\nto operate on the repo.\n\n**This is the default**, and this option has no effect, unless the [git.colocate config]\nis set to `false`.\n\nThis option is mutually exclusive with `--git-repo`.", Value: "--colocate", ConflictingFlags: []string{"--no-colocate", "--git-repo"}},
								{Name: "no-colocate", Description: "Disable colocation of the Jujutsu repo with the git repo\n\nPrevent Git tools that are unaware of `jj` and regular Git commands from operating on the\nrepo. The Git repository that stores most of the repo data will be hidden inside a\nsub-directory of the `.jj` directory.\n\nSee [colocation docs] for some minor advantages of non-colocated workspaces.\n\n[colocation docs]:\nhttps://docs.jj-vcs.dev/latest/git-compatibility/#colocated-jujutsugit-repos", Value: "--no-colocate", ConflictingFlags: []string{"--colocate", "--git-repo"}},
								{Name: "git-repo", Description: "Specifies a path to an **existing** git repository to be used as the backing git repo for\nthe newly created `jj` repo.\n\nIf the specified `--git-repo` path happens to be the same as the `jj` repo path (both .jj\nand .git directories are in the same working directory), then both `jj` and `git` commands\nwill work on the same repo. This is called a colocated workspace.\n\nThis option is mutually exclusive with `--colocate`, and so if passed, turns colocation\noff.", Value: "--git-repo", RequiresInput: true, ConflictingFlags: []string{"--colocate", "--no-colocate"}, InputType: "GIT_REPO"},
							},
						},
					},
				},
